...
from models.ege.model_ege import RecEGE
from models.ege.eval_utils import evaluation
from models.ege.train_utils import train_sl
...
def train_rl(epoch, optimizer):
    ...
    for batch_idx in range(1, num_epoch + 1):
        ...
        for k in range(dialog_turns):
            ...
            # Q-value
            for kk in range(len(hist_feedback)):
                value = model.dis_merge_forward(state, hist_feedback[kk])
                if kk==0:
                    value_arr = torch.reshape(value, (args.batch_size, 1))
                else:
                    value_arr = torch.cat((value_arr, value),1)
            value = torch.max(value_arr, dim=1).values
