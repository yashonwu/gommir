...
def evaluation(split, model, user, ranker, batch_size, top_k, turns, model_dir, result_folder):
    ...
    implicit_act_img_idx = torch.LongTensor(batch_size,top_k-1)
    ...
    for batch_idx in range(1, total_batch + 1):
        ...
        for k in range(dialog_turns):
            ...
            act_img_idx, position, n_act_img_idx, n_position,implicit_act_img_idx = ranker.nearest_neighbor_selector_implicit(user_img_idx, top_k_act_img_idx)
            act_input = all_input[act_img_idx]
            
            # act_img_idx = ranker.nearest_neighbor_selector(user_img_idx, top_k_act_img_idx)
            # act_img_name = image_name(act_img_idx, split)
            
            txt_input, sents = user.get_feedback(act_img_idx, position, user_img_idx, split)
            # if torch.cuda.is_available():
            ...
            # output_dict["act_img_idx"]= act_img_idx.cpu().numpy()
            # output_dict["act_img_name"]= np.array(act_img_name)
            output_dict["text_feedback"]= np.array(sents)
            output_dict["position"] = np.array(position)
            ...
            ranking_candidate = ranker.compute_rank_correct(action.data, user_img_idx, memory_act_img_idx)

            # avoid repeated recommendations
            # print("******************************")
            # print("avoid repeated recommendations!")
            
            top_km_act_img_idx = ranker.k_nearest_neighbors(action.data,K=top_k*turns)
            ...
            new_act_emb = ranker.feat[top_k_act_img_idx]

            # new action embedding for next turn
            act_emb = new_act_emb
