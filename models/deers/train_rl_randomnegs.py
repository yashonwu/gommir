...
from models.deers.model_deers import RecDEERS
from models.deers.eval_utils import evaluation
from models.deers.train_utils import train_sl

parser = argparse.ArgumentParser(description='MMCRS')

# user simulator
parser.add_argument('--data-type', type=str, default="",
                    help='dress, shirt, toptee,shoe')
...
# get log prob
def dist(state, user_img_idx, top_k_act_img_idx, memory_act_img_idx, split):
...
def train_rl(epoch, optimizer):
    ...
    for batch_idx in range(1, num_epoch + 1):
        ...
        user.sample_k_idx(top_k_act_img_idx, split, top_k=args.top_k)
        ...
        for k in range(dialog_turns):
            ...
            if torch.cuda.is_available():
                implicit_act_img_idx = implicit_act_img_idx.cuda()
            implicit_act_emb = ranker.feat[implicit_act_img_idx]
            
            # update the query action vector given feedback image and text feedback in this turn
            state_pos, state_neg = model.merge_forward(act_input, txt_input, implicit_act_emb)
            ...
            # log_prob, reward, Q value
            log_prob, reward = dist(state, user_img_idx, top_k_act_img_idx, implicit_memory_act_img_idx,split)
        
            # get q values with top_k_act_img
            if torch.cuda.is_available():
                top_k_act_img_idx = top_k_act_img_idx.cuda()
            act_emb = ranker.feat[top_k_act_img_idx]
            
            for kk in range(args.top_k):
                value = model.forward_qvalue(state_pos, state_neg, act_emb[:,kk,:])
                if kk==0:
                    value_arr = torch.reshape(value, (args.batch_size, 1))
                else:
                    value_arr = torch.cat((value_arr, value),1)
            # print(value_arr)
            value = torch.mean(value_arr, dim=1)
        ...
        for i, prob_temp in enumerate(prob_list):
            ...
            pred_value = value_list[i]
            targ_value = reward_list[i] + args.gamma * value_list[i+1]
            value_loss = F.smooth_l1_loss(pred_value, targ_value.detach())
            policy_loss = log_prob + value_loss
...
for epoch in range(1, args.epochs + 1):
    ...
    ndcg_mean, sr_mean, ndcg, sr = evaluation('val', model, user, ranker, args.batch_size, args.top_k, args.val_turns, args.model_dir, args.result_folder)
