...
def dist(state, combined_emb, user_img_idx, top_k_act_img_idx, memory_act_img_idx):
    ...
    pos_emb = ranker.feat[p_act_img_idx]
    sum_reward = torch.sum(pos_emb*target_emb, dim=1)
...
def train_rl(epoch, optimizer):
    ...
    for batch_idx in range(1, num_epoch + 1):
        ...
        for k in range(dialog_turns):
            ...
            log_prob, reward, trip_loss = dist(state, combined_emb, user_img_idx, top_k_act_img_idx, implicit_memory_act_img_idx)
