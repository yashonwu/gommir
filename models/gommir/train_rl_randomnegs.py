...
from models.gommir.model_transformer import RecTransformer
from models.gommir.eval_utils import evaluation
...
# # get log prob with l2 norm
# def dist(state, combined_emb, user_img_idx, top_k_act_img_idx, memory_act_img_idx):
...
#     trip_loss = triplet_loss.forward(combined_emb, target_emb, neg_emb)

#     return log_prob, sum_reward, trip_loss


# get log prob
def dist(state, combined_emb, user_img_idx, top_k_act_img_idx, memory_act_img_idx, split):
    if torch.cuda.is_available():
        user_img_idx = user_img_idx.cuda()
        top_k_act_img_idx = top_k_act_img_idx.cuda()
    target_emb = ranker.feat[user_img_idx]
...
def train_rl(epoch, optimizer):
    ...
    for batch_idx in range(1, num_epoch + 1):
        ...
        for k in range(dialog_turns):
            ...
            log_prob, reward, trip_loss = dist(state, combined_emb, user_img_idx, top_k_act_img_idx, implicit_memory_act_img_idx, split)
            
            prob_list.append(log_prob)
            reward_list.append(reward)
            trip_list.append(trip_loss)
        ...
        for i, prob_temp in enumerate(reversed_prob_list):
            ...
            loss_sum = policy_loss + loss_sum + trip_list[dialog_turns-1-i]
